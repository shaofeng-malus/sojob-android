...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    protected LinkedList<View> mLoadedViews;
    protected int mCurrentBufferIndex;
    protected int mCurrentAdapterIndex;
    ...
    protected FlowIndicator mIndicator;
...
//				Logs.e("TAG","VIEWFLOW"+mLoadedViews.size());

            }
        };
        handler.postDelayed(post, timeSpan);
//		Message message = handler.obtainMessage(0);
//		handler.sendMessageDelayed(message, timeSpan);
    }

    private PostImg post = new PostImg();

    private class PostImg implements Runnable {
        @Override
        public void run() {
            if (handler == null || mLoadedViews.size() == 3) {
                return;
            }
...
//				handler.removeMessages(0);
    ...
    private void postViewSwitched(int direction) {
        ...
        if (direction > 0) { // to the right
            ...
            if (null != mAdapter) {//Frozen 解决mAdapter.getCount()报空指针
